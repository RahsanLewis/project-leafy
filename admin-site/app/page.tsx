...
export default function Home() {
  ...
  const refresh = useCallback(async (search = "") => {
    ...
    try {
      const [counts, result] = await Promise.all([
        api({ action: "summary" }),
        tab === "queue" ? api({ action: "list", status: "pending_review", query: search }) : tab === "foods" ? api({ action: "search_foods", query: search }) : api({ action: "search_additives", query: search }),
      ]);
      setSummary(counts);
  ...
  async function open(row: Row) {
    ...
    try {
      const result = tab === "queue" ? await api({ action: "detail", contribution_id: row.id }) : tab === "foods" ? await api({ action: "food_detail", food_version_id: row.food_version_id }) : await api({ action: "additive_detail", canonical_id: row.canonical_id });
  ...
  return (
    <main className="shell">
      <aside className="sidebar">
        ...
        <nav>
          <button className={tab === "queue" ? "active" : ""} onClick={() => setTab("queue")}>Review queue <span>{summary.pending ?? "—"}</span></button>
      ...
      <section className="workspace">
        <header><div><p className="eyebrow">Leafy operations</p><h1>{tab === "queue" ? "Product review" : tab === "foods" ? "Food database" : "PFQS additive registry"}</h1><p>{tab === "queue" ? "Verify community label submissions before they become shared catalog records." : tab === "foods" ? "Search active packaged and unpackaged food records." : "Search the exact versioned registry used by the PFQS scorer."}</p></div></header>
        <form className="search" onSubmit={(event) => { event.preventDefault(); void refresh(query); }}><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={tab === "additives" ? "Name, alias, E-number, family, or tier" : "Name, brand, or barcode"}/><button>Search</button></form>
...
function ResultRow({ row, tab, active, onClick }: { row: Row; tab: Tab; active: boolean; onClick: () => void }) {
  ...
  const subtitle = tab === "queue" ? `${row.confirmed_fields?.brand_name || "Brand not shown"} · ${date(row.last_submitted_at)}` : tab === "foods" ? `${row.brand_name || "No brand"} · ${row.gtin || row.market_country || "No barcode"}` : `${row.family || "Uncategorized"} · ${maxTier(row)} tier`;
  return <button className={`result ${active ? "selected" : ""}`} onClick={onClick}><div><b>{title}</b><small>{subtitle}</small></div><span>›</span></button>;
}
...
function ReviewDetail({ data, reason, setReason, moderate, loading }: any) {
  ...
  return <article><div className="detail-heading"><div><p className="eyebrow">Pending review</p><h2>{fields.product_name || data.gtin}</h2><p>{fields.brand_name || "Brand not shown"} · {data.gtin}</p></div></div>
...
function Empty({ tab }: { tab: Tab }) { return <div className="empty-list"><b>{tab === "queue" ? "Queue clear" : "No results"}</b><p>{tab === "queue" ? "There are no products waiting for review." : "Try a broader search."}</p></div>; }
