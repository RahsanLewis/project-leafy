...
export const adaptiveModelVersion = 'leafy-energy-balance-v1'
...
export function adaptiveCandidate(
  ...
  const weightSlope = theilSenSlope(weights)
  ...
  if (input.goal !== 'maintain' && input.target_weight_kg != null && weeklyChange > 0) {
    const currentWeight = [...weights].sort((a, b) => b.recorded_on.localeCompare(a.recorded_on))[0].weight_kg
