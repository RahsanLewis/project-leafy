...
export function calculate(input: Input, now = new Date()) {
  ...
  if (age < 18 || age > 100) {
    throw new Error("Leafy currently supports adults ages 18 through 100.");
  }
  if (input.height_cm < 120 || input.height_cm > 230) {
    throw new Error("Height is outside the supported range.");
  }
  if (input.current_weight_kg < 35 || input.current_weight_kg > 350) {
    throw new Error("Weight is outside the supported range.");
  }
  if (input.goal !== "maintain") {
    const target = input.target_weight_kg;
    if (!target || target < 35 || target > 350) {
      throw new Error("A valid target weight is required.");
    }
    if (
      (input.goal === "lose" && target >= input.current_weight_kg) ||
      (input.goal === "gain" && target <= input.current_weight_kg)
    ) throw new Error("Target weight conflicts with the selected goal.");
    const currentBMI = input.current_weight_kg /
      Math.pow(input.height_cm / 100, 2);
    const targetBMI = target / Math.pow(input.height_cm / 100, 2);
    if (input.goal === "lose" && (currentBMI < 18.5 || targetBMI < 18.5)) {
      throw new Error(
        "Leafy cannot create a weight-loss plan below a BMI of 18.5.",
      );
    }
  }

  const bmr = 10 * input.current_weight_kg + 6.25 * input.height_cm - 5 * age +
    (input.calculation_sex === "male" ? 5 : -161);
  ...
  if (input.goal === "lose") {
    ...
    if (tdee - targetCalories < 50) {
      throw new Error(
        "Leafy cannot create a meaningful generic deficit from these inputs.",
      );
    }
  }
  const calories = Math.round(targetCalories / 10) * 10;
  const referenceWeight = input.goal === "maintain"
    ? input.current_weight_kg
    : input.target_weight_kg!;
  const desiredProteinCalories = referenceWeight *
    (input.goal === "maintain" ? 1.2 : 1.6) * 4;
  const proteinCalories = Math.min(
    Math.max(desiredProteinCalories, calories * 0.10),
    calories * 0.35,
  );
  ...
  if (calories - proteinCalories - fatCalories < minimumCarbohydrateCalories) {
    fatCalories = Math.max(
      calories * 0.20,
      calories - proteinCalories - minimumCarbohydrateCalories,
    );
  }
  ...
  if (input.goal !== "maintain" && weeklyChange > 0) {
    const days = Math.ceil(
      Math.abs(input.target_weight_kg! - input.current_weight_kg) /
        weeklyChange * 7,
    );
  ...
  return {
    ...
    carbohydrate_g: Math.round(carbohydrateCalories / 4),
